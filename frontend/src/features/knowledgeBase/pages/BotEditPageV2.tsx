...
const BotEditPageV2: React.FC = () => {
  ...
  const [analyzer, setAnalyzer] = useState<string>(
    DEFAULT_OPENSEARCH_ANALYZER[i18n.language] ?? 'icu'
  );
  ...
  const isValid = useCallback((): boolean => {
    ...
    // S3 URLs validation - s3://bucket-name/
                ...
                {!isNewBot && (
                  <div className="mt-3 grid gap-1">
                    ...
                    <div
                      ...
                      <div>{openSearchParams.analyzer.tokenizer}</div>
                      <div>
                        {t(
                          'knowledgeBaseSettings.opensearchAnalyzer.normalizer'
                        )}
                      </div>
                      <div>{openSearchParams.analyzer.characterFilters}</div>
                      <div>
                        {t(
